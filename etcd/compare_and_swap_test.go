...
func TestCompareAndSwap(t *testing.T) {
	...
	// This should succeed
	resp, err := c.CompareAndSwap("foo", "bar2", "bar", 0)
	if err != nil {
		t.Fatal(err)
	}
	if !(resp.Node.Value == "bar2" && resp.Node.Key == "/foo") {
		t.Fatalf("CompareAndSwap 1 failed: %#v", resp)
	}

	if !(resp.PrevNode.Value == "bar" && resp.PrevNode.Key == "/foo") {
		t.Fatalf("CompareAndSwap 1 prevNode failed: %#v", resp)
	}

	// This should fail because it gives an incorrect prevValue
	resp, err = c.CompareAndSwap("foo", "bar3", "xxx", 0)
	if err == nil {
	...
	// This should succeed
	resp, err = c.CompareAndSwap("foo", "bar2", "", resp.Node.ModifiedIndex)
	if err != nil {
		t.Fatal(err)
	}
	if !(resp.Node.Value == "bar2" && resp.Node.Key == "/foo") {
	...
	resp, err = c.CompareAndSwap("foo", "bar3", "", 29817514)
	if err == nil {
