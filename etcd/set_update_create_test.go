...
func TestUpdate(t *testing.T) {
	...
	// This should succeed.
	resp, err = c.Update("foo", "wakawaka")
	if err != nil {
		t.Fatal(err)
	}
...
func TestCreate(t *testing.T) {
	...
	// This should succeed
	resp, err := c.Create(newKey, newValue)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCreateInOrder(t *testing.T) {
	...
	resp, err := c.CreateInOrder(dir, "1")
	if err != nil {
		t.Fatal(err)
	}
	...
	resp, err = c.CreateInOrder(dir, "2")
	if err != nil {
		t.Fatal(err)
	}
...
func TestUpdateDir(t *testing.T) {
	...
	// This should fail because the key does not exist.
	resp, err = c.UpdateDir("nonexistentDir")
	if err == nil {
		t.Fatalf("The key %v did not exist, so the update should have failed."+
			"The response was: %#v", resp.Node.Key, resp)
	}
