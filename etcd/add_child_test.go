...
func TestAddChild(t *testing.T) {
	...
	_, err := c.AddChild("fooDir", "v0")
	if err != nil {
		t.Fatal(err)
	}

	_, err = c.AddChild("fooDir", "v1")
	if err != nil {
		t.Fatal(err)
	}
	...
	resp, err = c.AddChild("nonexistentDir", "foo")
	if err != nil {
		t.Fatal(err)
	}
}
...
func TestAddChildDir(t *testing.T) {
	...
	// Creating a child under a nonexistent directory should succeed.
	// The directory should be created.
	resp, err = c.AddChildDir("nonexistentDir")
	if err != nil {
		t.Fatal(err)
	}
}
